...
<div id="app">
  ...
  <div class="flex mx-auto items-end mb-2.5 mt-6 justify-center items-center gap-4 sm:flex-col sm:gap-2 sm:w-10/12">
    <my-select class="sm:w-full" v-model="selectedOption" :options="sortOptions" :searchBy="sortBy"/>
    <my-select class="sm:w-full" v-model="selectedEpisode" :options="getData(locationList)" :searchBy="filterBy"/>
  </div>

  <div class="flex mx-auto items-end mb-2.5 justify-center items-center gap-4 sm:flex-col sm:gap-2 sm:w-10/12">
      <my-button class="sm:order-3 sm:w-full" @click="resetAllFilters">Reset all</my-button>
      <my-input class="block sm:w-full" v-model.trim="searchedName" @keydown.enter="searchedByName"/>
...
export default defineComponent({
  name: 'App',
  components: {
    MyHeader, MyContent, MySelect,MyInput,MyButton,MyFooter
  },
  data() {
    return {
      cards: [],
  ...
  methods: {
    ...
    getData (arrArrs:string[]){
      ...
      return newArr.map(val => ({['name'] : val.toString(),['value'] : val.toString()}));
    },

    // поиск по имени
    searchedByName() {
      this.cards = this.defaultCards.filter((el) => el['name'].toLowerCase().includes(this.searchedName.toLowerCase()));
    },
  ...
  watch: {
    ...
    selectedEpisode(newValue:string) {
      if(newValue){
        ...
        this.cards = this.cards.filter((el:any) => el['location']['name'] === newValue);
        this.searchedName = '';
      }
    },
  },
    ...
    sortedCards(): any {
      if (this.selectedOption !== 'firstEpisodeOfCharacter') {
        return [...this.cards].sort((card1: any, card2: any) => {
          return card1[this.selectedOption]?.localeCompare(card2[this.selectedOption])
        })
      } else {
        return [...this.cards].sort((card1: any, card2: any) => {
